...
@bot.on_message(filters.command(["getkey", "/getkey"]))
async def key(bot, message):
    res = requests.get('https://free.scarcheats.xyz/redirect.php')
    s = (res.content)
    g = (res.text)
    r = requests.get('https://api.github.com/events', stream = True)
    await message.reply("YOUR KEY IS :-",g[-207:-189])
